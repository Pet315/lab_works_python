...
class ICourseFactory(ABC):
    ...
    @abstractmethod
    def add_teacher(self, t_name, course):
        pass

    @abstractmethod
    def add_course(self, name, teacher, program, type_course):
        pass
...
class CourseFactory(ICourseFactory):
    ...
    def add_teacher(self, t_name, c_subjects):
        if not c_subjects:
            raise ValueError("Error 5")
        try:
            with open('case1.json', 'w') as file:
                self.t_dict.update({t_name: Teacher(t_name, c_subjects).__dict__})
                json.dump(self.t_dict, file, indent=2)
        except FileNotFoundError:
            raise FileNotFoundError("Error 6")
        return Teacher(t_name, c_subjects)

    def add_course(self, name_surname, t_code, program, c_type):
        ...
        self.add_teacher(t_code.t_name, t_code.c_name)
        self.c_dict = json.load(open("case2.json"))
        try:
            with open('case2.json', 'w') as file:
                self.c_dict.update({name_surname: dict_of_courses[c_type].__dict__})
                json.dump(self.c_dict, file, indent=2)
        except FileNotFoundError:
            raise FileNotFoundError("Error 6")
...
a = CourseFactory()
x1 = a.add_teacher('John Smitt', ['Chemistry', 'Biology', 'Physics', 'Math'])
x2 = a.add_teacher('Viktor Mitlov', ['Higher mathematics', 'Physics', 'Electrical engineering'])
y1 = a.add_course('Biology', x1, ['mitosis', 'meiosis', 'virusology', 'zoology'], 'Offsite')
y2 = a.add_course('Physics', x2, ['mechanics', 'dynamics', 'electrostatics'], 'Local')
